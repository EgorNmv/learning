...
import { findAcceptedFeedbacksByTrainingId } from "./queries/findAcceptedFeedbacksByTrainingId";
...
export class FeedbackResolver {
  ...
  public async feedback(@Ctx() { connection }: Context, @Arg("id") id: number) {
    return await findFeedbackById(connection, id);
  }

  @Query(() => [FeedbackEntity])
  public async feedbacks(@Ctx() { connection }: Context) {
  ...
  public async feedbacksByTrainingId(
    ...
    @Arg("feedbackType", { description: "1 - рекомендация, 2 - отзыв" })
    feedbackType: number
  ) {
    return await findFeedbacksByTrainingId(
      connection,
      trainingId,
      feedbackType
    );
  ...
  @Query(() => [FeedbackEntity])
  public async acceptedFeedbacksByTrainingId(
    @Ctx() { connection }: Context,
    @Arg("trainingId") trainingId: number,
    @Arg("feedbackType", { description: "1 - рекомендация, 2 - отзыв" })
    feedbackType: number
  ) {
    return await findAcceptedFeedbacksByTrainingId(
      connection,
      trainingId,
      feedbackType
    );
